...
@fig-g3-studytime reveals a slight increase in the typical final grade as amount of study time increases, although not substantial enough to be among the strongest factors.
...
edu_achievement |>
  mutate(studytime = factor(studytime, labels = new_labels)) |>
  ggplot(
    aes(
      x = studytime,
      y = G3,
...
@fig-medu displays how there is a variable trend of student performance with regard to parent education level. The median final grade for students whose parent has no education is higher than the rest, but there are only a few points of data where parent education is none, making the pattern difficult to confirm. Nonetheless, the trend is not strong enough to be considered one of the strongest factors of student performance.
...
ggplot(edu_long, aes(x = factor(education), y = G3, fill = factor(education))) +
  ...
  labs(
    ...
    subtitle = "Student Performance in Math in the Portuguese schools of\nGabriel Pereira and Mousinho da Silveira"
  ) +
  theme_minimal() +
  theme(legend.position = "none")
```
