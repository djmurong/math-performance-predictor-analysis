...
edu_achievement_longer |>
  mutate(
    ...
    alc_lvl = fct_relevel(as.character(alc_lvl))
  ) |>
...
ggplot(edu_achievement, aes(x = Fedu, y = G3, group = Fedu, color = Fedu)) + 
  geom_jitter() +
  geom_boxplot(alpha = 0.3) +
  labs(
...
Gabriel Pereira and Mousinho da Silveira"
  ...
  theme(legend.position = "none")
```
Medu and G3 correlation:
```{r}
#| label: medu-summarize
edu_achievement |>
  summarize(r = cor(Medu, G3))
```

Fedu and G3 correlation:
```{r}
#| label: fedu-summarize
edu_achievement |>
  summarize(r = cor(Fedu, G3))
```
...
ggplot(edu_achievement, aes(x = factor(health), y = G3, fill = factor(health))) +
  geom_boxplot(alpha = 0.7) +
  labs(
    title = "Distribution of Final Grades by Health Status",
    x = "Health (1 = Poor, 5 = Excellent)",
    y = "Final Grade (G3)"
  ) +
  theme_minimal() +
  theme(legend.position = "none")
...
```{r}
#| label: health-summarize
edu_achievement |>
  summarize(r = cor(health, G3))
```
...
```{r}
ggplot(edu_achievement, aes(x = factor(famrel), y = G3, fill = factor(famrel))) +
  geom_boxplot(alpha = 0.7) +
  labs(
    title = "Distribution of Final Grades by Family Relationship Strength",
    x = "Family Relationship (1 = Very Bad, 5 = Excellent)",
    y = "Final Grade (G3)"
  ) +
  theme_minimal() +
  theme(legend.position = "none")
...
```{r}
#| label: famrel-summarize
edu_achievement |>
  summarize(r = cor(famrel, G3))
```
...
#| label: fig-paid-g3
#| message: false
ggplot(edu_achievement, aes(x = paid, y = G3, fill = paid)) +
  geom_boxplot() +
  labs(
    title = "Relationship Between Paid Classes and Final Grade",
    x = "Taking Paid Classes",
    y = "Final Grade") +
  theme_minimal(base_size = 14) +
  theme(legend.position = "none")
```
...
ggplot(
  ...
  geom_boxplot() +
  labs(
    title = "Relationship Between Free Time and Final Grade",
    x = "Amount of Free Time",
    y = "Final Grade") +
  theme_minimal(base_size = 14) +
  theme(legend.position = "none")
```
...
```{r}
#| label: freetime-summarize
edu_achievement |>
  summarize(r = cor(freetime, G3))
```
...
ggplot(edu_achievement, aes(x = higher, y = G3, fill = higher)) +
  geom_boxplot(alpha=0.7) +
  labs(
    title = "Relationship Between Higher Education Interest\nand Final Grade",
